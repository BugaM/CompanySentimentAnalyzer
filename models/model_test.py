...
def test_minimal_accuracy():
    ...
    accuracy = load_accuracy.compute(predictions=predicted, references=labels)["accuracy"]
    print(f"accuracy: {accuracy}")
    assert accuracy >= MINIMAL_ALL

def test_minimal_recall():
    load_recall = evaluate.load("recall")
    recall = load_recall.compute(predictions=predicted, references=labels, average="macro")["recall"]
    print(f"recall: {recall}")
    assert recall >= MINIMAL_ALL

def test_minimal_f1():
    load_f1 = evaluate.load("f1")
    f1 = load_f1.compute(predictions=predicted, references=labels, average="macro")["f1"]
    print(f"f1: {f1}")
    assert f1 >= MINIMAL_ALL

def test_minimal_precision():
    load_precision = evaluate.load("precision")
    precision = load_precision.compute(predictions=predicted, references=labels, average="macro")["precision"]
